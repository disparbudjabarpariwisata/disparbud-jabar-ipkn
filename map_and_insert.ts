...
async function run() {
    // 1. Read SQL dump for mapping
    const dumpContent = fs.readFileSync('backups1/IPKN_Database_Backup_2026-02-26 (1) (1).sql', 'utf8');

    // Extact INSERT statements for survey_questions
    const oldIdToText: Record<string, string> = {};
    const regex = /INSERT INTO "survey_questions" \("id", "role_id", "question_text", [^\)]+\) VALUES \('([^']+)',\s*'([^']+)',\s*'([^']+)'/g;
    ...
    console.log(`Extracted \${Object.keys(oldIdToText).length} question texts from SQL dump.`);

    // 2. Fetch current questions from Supabase for this role
    const { data: currentQuestions, error: errQ } = await supabase
        .from('survey_questions')
        .select('id, question_text, institution_name')
        .eq('role_id', 'a6566276-64f2-4a06-bed8-b4beac1565e5');

    if (errQ) {
        console.error("Error fetching current questions:", errQ);
        return;
    }

    console.log(`Fetched \${currentQuestions?.length} current questions from Supabase.`);
    ...
    for (const d of suppliedData) {
        ...
        let oldText = oldIdToText[oldId];

        if (!oldText) {
            console.warn(`Could not find text in SQL dump for old_id \${oldId}. Will fallback to using oldId directly (might fail FK)`);
            oldText = "UNKNOWN";
        } else {
            // Replace escaped newlines or quotes in dump format
            oldText = oldText.replace(/\\n/g, '\n').replace(/''/g, "'");
        }

        let newId = oldId; // default to oldId if we can't map

        if (oldText !== "UNKNOWN") {
            // Find current question with identical text
            // Sometimes there are multiples (e.g., specific institution questions)
            // If there's an exact match in institution_name, prefer it, otherwise use the generic one or any match
            const matches = currentQuestions!.filter(q => q.question_text === oldText);

            if (matches.length > 0) {
                // Try finding one specifically for Biro Perekonomian or generic
                const specific = matches.find(q => q.institution_name === 'Biro Perekonomian');
                const generic = matches.find(q => !q.institution_name || q.institution_name === '' || q.institution_name === 'Umum');

                if (specific) newId = specific.id;
                else if (generic) newId = generic.id;
                else newId = matches[0].id; // Fallback to first available

                mappedCount++;
            } else {
                console.warn(`Found old text but no matching new question in DB for: "\${oldText.substring(0, 30)}..."`);
            }
        }
    ...
    console.log(`Successfully mapped \${mappedCount} out of \${suppliedData.length} questions.`);

    // Clean existing answers just in case
    await supabase.from('survey_answers').delete().eq('respondent_id', respondentId);

    // Insert answers
    const { error: insErr } = await supabase.from('survey_answers').insert(toInsert);
    if (insErr) {
        console.error("Failed to insert answers. Probably FK missing:", insErr);
    } else {
